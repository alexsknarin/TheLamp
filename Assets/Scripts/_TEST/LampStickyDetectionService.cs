...
public class LampStickyDetectionService : MonoBehaviour, IInitializable
{
    ...
    [SerializeField] private float _stickyExitRadius = 0.5f;
    ...
    private void CheckStickables()
    {
        foreach (var stickable in _stickables)
        {
            ...
            if (stickable.StickState == StickableState.Outside &&
                distance < attackZoneCombinedRadius)
            {
                Debug.Log("Attack zone enter Detected");
                Debug.Log($"EnemyType: {stickable.GetType()}");
                stickable.HandleEnterAttackZone();
            }
            
            // Exiting Attack Zone Before Stick Because of Damage
            if (stickable.StickState == StickableState.InAttackZoneDamaged &&
                distance > attackZoneCombinedRadius)
            {
                Debug.Log("Attack zone exit before stick Detected");
                Debug.Log($"EnemyType: {stickable.GetType()}");
            ...
            // Stick detection
            if (stickable.StickState == StickableState.InAttackZone &&
                distance < _combinedStickRadius + stickable.Radius)
            {
                Debug.Log("Sticky Collision detected");
                Debug.Log($"EnemyType: {stickable.GetType()}");
                stickable.HandleStick(transform);
            }
            
            // Exiting Stick Zone
            if (stickable.StickState == StickableState.Sticked &&
                distance > _stickyExitRadius + stickable.Radius)
            {
                Debug.Log("Sticky exit Detected");
                Debug.Log($"EnemyType: {stickable.GetType()}");
                _stickablesToRemove.Add(stickable);
                stickable.HandleExitAttackZone();
            }
